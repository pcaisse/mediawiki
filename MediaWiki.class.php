...
class MediaWiki {
	...
	*/
	...
	static private function _loginUser($user, $pass, $token) {
		...
		* If login is successful, relevant user/session data is returned.
		...
		*/
		...
		if (!isset($resultVal)) {
			throw new Exception("Error reading login result");
		}
		if (isset($resultVal) && $resultVal !== "Success") {
			return null;
		}
	
    	// get and return login info since user has logged in successfully
